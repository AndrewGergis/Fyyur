...
def venues():
    ...
    for venue in venue_list:
        shows_list = Show.query.filter(Show.start_time > current_time).filter_by(venue_id=venue.id).all()
        print(shows_list)
        num_upcoming_shows = len(shows_list)
        print(data)
        existing = list(filter(lambda x: x["city"] == venue.city and x["state"] == venue.state, data))
        print(existing)
        if existing:
            venue_data = {
                "id": venue.id,
            ...
            existing[0]["venues"].append(venue_data)
        else:
            venue_data = {
                "city": venue.city,
                "state": venue.state,
...
def search_venues():
    ...
    print(search_query)
    venues_results = Venue.query.filter(Venue.name.ilike(f'%{search_query}%')).all()
    ...
    for venue in venues_results:
        shows_list = Show.query.filter(Show.start_time > datetime.utcnow()).filter_by(venue_id=venue.id).all()
        print(shows_list)
        num_upcoming_shows = len(shows_list)
...
def show_venue(venue_id):
    ...
    print(venue)
    upcoming_shows = Show.query.filter(Show.start_time > datetime.utcnow()).filter_by(venue_id=venue.id).all()
    print(upcoming_shows)
    num_upcoming_shows = len(upcoming_shows)

    past_shows = Show.query.filter(Show.start_time < datetime.utcnow()).filter_by(venue_id=venue.id).all()
    print(upcoming_shows)
...
def show_artist(artist_id):
    ...
    print(artist)
    upcoming_shows = Show.query.filter(Show.start_time > datetime.utcnow()).filter_by(artist_id=artist.id).all()
    print(upcoming_shows)
    num_upcoming_shows = len(upcoming_shows)

    past_shows = Show.query.filter(Show.start_time < datetime.utcnow()).filter_by(artist_id=artist.id).all()
    print(upcoming_shows)
    num_past_shows = len(past_shows)
